...
/* ======================= Remote EEPROM reset (Firebase) ======================= */
/*
  The website writes data/<veh>/reset_eeprom = 1 to ask this device to wipe its
  config and reboot into the AP setup portal (so a new truck can be configured).

  IMPORTANT: we clear the flag back to 0 in Firebase BEFORE wiping EEPROM and
  restarting. After the wipe this device loses its vehnum + WiFi credentials and
  can no longer reach Firebase, so if we did not clear the flag now it would stay
  at 1 and the device would reset itself again the next time it is reconfigured
  with the same vehicle number.
*/
void checkResetFromFirebase() {
  ...
  String url = base + "data/" + urlEncode(veh) + "/reset_eeprom.json";
  ...
  if (getFromFirebase(url, resp)) {
    ...
    if (resp == "1") {
      Serial.println("Remote EEPROM reset command received from Firebase!");

      // Clear the flag first (see note above) so we don't loop-reset later.
      putToFirebase(url, "0");

      // Audible + visual feedback: short beep and RED LED.
      digitalWrite(PIN_BUZZER, HIGH);
      setColor(true, false);
      delay(500);
      digitalWrite(PIN_BUZZER, LOW);

      // Wipe config and reboot -> device comes up in AP setup mode.
      clearConfigEEPROM();
      delay(300);
      ESP.restart();
    }
  } else {
    Serial.println("Failed to GET reset_eeprom status from Firebase");
  }
}
